...
describe("SimilarityThresholdSetting", () => {
    ...
    // The commit path (mask, rollback on failure, unchanged-value guard) is covered by
    // tests/unit/model/project-setting.svelte.test.ts, which can call `commit()` directly
    // instead of driving the slider through jsdom.
